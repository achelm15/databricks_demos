...
This demo explains why, shows what to do about it today with PySpark/SQL, and demonstrates the long-term answer with DLT expectations.
...
| `02_pk_unique_constraints` | Add PK, UNIQUE, and FK constraints; verify via `information_schema` | `ALTER TABLE ADD CONSTRAINT` |
...
| `05_dlt_enforce_uniqueness` | **DLT expectations** — centralized enforcement that actually stops bad data | Engine-level PK/UNIQUE constraints |
...
| Triggers fire on bad data | DLT `expect_or_drop` / `expect_or_fail` (day 2) |
...
## DLT Enforcement by Layer

| Layer | Enforcement | Why |
|-------|-------------|-----|
| **Bronze** | None | Raw landing zone — keep everything for debugging upstream issues |
| **Silver** | `expect_or_drop` | Filter nulls, bad formats, obvious junk (like CHECK constraints + triggers) |
...
3. **Day 2: DLT expectations** give you centralized, declarative enforcement that can't be bypassed — the lakehouse equivalent of engine-level constraints
